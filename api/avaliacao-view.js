...
async function gerarAvaliacao() {
  ...
  try {
    const supa = window.supabase.createClient('https://lgugjikghdelbqlxmhqd.supabase.co', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxndWdqaWtnaGRlbGJxbHhtaHFkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIxMjg4NzUsImV4cCI6MjA4NzcwNDg3NX0.WS9WTMwseWEKZHP8N4th3qfnrdaNCu2Ii2q-yadiU7E');
    const { data: { session } } = await supa.auth.getSession();
    if (session) {
      const { data: perfil } = await supa.from('perfis').select('plano,usos_avaliacao').eq('id', session.user.id).single();
      if (perfil && perfil.plano === 'gratuito' && (perfil.usos_avaliacao || 0) >= 1) {
        alert('Você atingiu o limite de 1 avaliação do plano gratuito.\\n\\nAcesse o site principal para assinar o plano Professor (R$ 9,90/mês) e gerar avaliações ilimitadas!');
        return;
      }
    }
...
Gere exatamente \${qtd} questões seguindo as regras. Responda APENAS com o JSON, nenhum texto adicional.\`;
    ...
    try {
      ...
      const { data: { session } } = await supa.auth.getSession();
      if (session) {
        const { data: p } = await supa.from('perfis').select('plano,usos_avaliacao').eq('id', session.user.id).single();
        if (p && p.plano === 'gratuito') {
          await supa.from('perfis').update({ usos_avaliacao: (p.usos_avaliacao || 0) + 1 }).eq('id', session.user.id);
        }
      }
    } catch(e) {}
...
async function exportarPDF() {
  ...
  doc.text('(*) Preencha completamente a bolinha    (X) Nao rasure    Marque apenas UMA alternativa por questao', M, y);
  y += 10;
